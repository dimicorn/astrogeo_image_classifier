...
class Table(object):
    def __init__(self, config, table_name: str) -> None:
    ...
    def connect2table(self) -> None:
        with connect(
            host=self.host, dbname=self.dbname, user=self.user, password=self.psswd
        ) as self.conn:
...
class Catalogue(Table):
    def create_table(self) -> None:
		...
		antennas int, antenna_tables int, uv_quality int, comment varchar(100));"
        self.cur.execute(create_table_query)
        self.conn.commit()

    def insert_value(self, values: tuple) -> None:
        insert_query = f"insert into {self.table_name}(object_name, obs_date, freq,\
