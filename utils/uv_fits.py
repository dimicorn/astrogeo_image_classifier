...
class UVFits(Fits):
    ...
    def getSQLParams(self, map_table_name: str, config) -> tuple:
        ...
        ampl = self._X[2]
        ampl_data = (float(np.min(ampl)), float(np.max(ampl)), 
                     float(np.mean(ampl)), float(np.median(ampl)))
    ...
    def qualityComment(
        ...
        with connect(
			...
			user=cfg.user, password=cfg.psswd) as conn:
                cur = conn.cursor()
        map_file = self.file_name.replace('vis', 'map')
        select_map = f"select map_quality from {map_table_name} where file_name = '{map_file}';"
