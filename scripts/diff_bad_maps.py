...
for source, file, comment in tqdm(zip(s_df_filt.object_name, s_df_filt.file_name, s_df_filt.comment)):
    draw(f'{root}/images_verApr2025/{source}/{file}', suptitle=comment, img_dir=s_diff_path)

for source, file, comment in tqdm(zip(x_df_filt.object_name, x_df_filt.file_name, x_df_filt.comment)):
    draw(f'{root}/images_verApr2025/{source}/{file}', suptitle=comment, img_dir=x_diff_path)