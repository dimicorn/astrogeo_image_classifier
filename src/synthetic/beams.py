...
class Beams(object):
    ...
    def two_points_beam(self, d: int, alpha: float, point: tuple = (0, 0)) -> np.array:
        x0, y0 = point
        point2 = (int(x0 + d * np.sin(alpha)), int(y0 + d * np.cos(alpha)))
        return self.point_beam(point) + self.point_beam(point2)
    
    def gauss_beam(self, b_maj: int, b_min: int, b_pa: float, 
                   point=(0, 0)) -> np.array:
        x0, y0 = point
    ...
    def two_gauss_beam(self, 
                       b_maj1: int, b_min1: int, b_pa1: float, point: tuple, 
                       b_maj2: int, b_min2: int, b_pa2: float, 
                       d: int, alpha: float) -> np.array:
        x0, y0 = point
        point2 = (int(x0 + d * np.sin(alpha)), int(y0 + d * np.cos(alpha)))
    ...
    def gauss_w_jet_beam(self, b_maj: int, b_min: int, b_pa: float,
                         d: int, alpha: float, point=(0, 0)) -> np.array:
    ...
    def gauss_w_two_jets_beam(self, b_maj: int, b_min: int, b_pa: float,
                              d: int, alpha: float, point=(0, 0)) -> np.array:
        x0, y0 = self.shape[0]//2 + point[0], self.shape[1]//2 + point[1]
    ...
    def gauss_w_spiral_beam(self, b_maj: int, b_min: int, b_pa: float,
                            v: float, c: float, w: float, phi: float, 
                            point=(0, 0)) -> np.array:
        x0, y0 = self.shape[0]//2 + point[0], self.shape[1]//2 + point[1]
